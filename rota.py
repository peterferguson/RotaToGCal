...
def gCalRota(rota: pd.DataFrame, 
             shift_map: dict,
             doctor_name: str = 'Laura Marmion'
            ) -> pd.DataFrame:
    """     
    params
    ...
    """
    name_on_calendar = next(
        name for name in rota.columns if doctor_name in name
        )
    doctor_rota = rota[name_on_calendar].to_frame().rename(
        columns={name_on_calendar: 'summary'}
        )
    ...
    doctor_rota['start_time'] = doctor_rota['Date'] \
        + doctor_rota['start_time'].apply(timedeltaString)
    doctor_rota['end_time'] = doctor_rota['Date'] \
        + doctor_rota['end_time'].apply(timedeltaString)
    doctor_rota.loc[
        doctor_rota['end_time'].lt(doctor_rota['start_time'])
        , 'end_time'] += timedelta(days=1)

    return doctor_rota

# TODO: This is the function which must be edited for each new rota
def orientateRota(rota_path: str):
    ...
    doctor_dict = {
        str(i+1): name  for i, name in enumerate([
        'Faryal Peerally (IMT1)', 'Dean Carolan (IMT1)', 'Tom Carson (F2)', 
        'Catherine Rooney (GP)', 'Sreelata Periketi (GP)', 
        'Dawn Pollock Shels (F2) & Samantha Banford (IMT1)', 
        'Laura Shannon (IMT2)', 'Vacant Line', 'Ismail Abdulazeez (GP)',
        'Hannah Stewart (IMT1)', 'Caoimhe Cooke (F2)', 'Roisin Chambers(GP)',
        'Ryan Doherty (F2)', 'Laura Marmion (F2)', 'Clare Moore (GP)', 
        'Gary Roulston (IMT1)', 'Opeyemi Makanjoula (IMT1)', 'Shwe Win (IMT2)',
        'Vacant Line', 'Chandrika Mary Kanjee (F2)', 'Razan Elnaim (F2)', 
        'Alice Cullen(Loc)', 'Craig Meek (Loc)', 'Peter Neeson (F2)',
        'Catriona Gormley (F2)', 'Rebekah Ross(CT2)', 'Rachael Henderson(GP)'
        ])
    }
