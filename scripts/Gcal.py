            ...
            json.dump({ "web":
                {
                    ...
                    "auth_provider_x509_cert_url": f["web"]["auth_provider_x509_cert_url"],
...
def createCalendar(service, summary):
    ...
    page_token = None
    while True:
        calendar_dict = service.calendarList().list(
            pageToken=page_token
            ).execute()
        if summary not in (
            calendar['summary'] for calendar in calendar_dict['items']
            ):
            new_calendar = service.calendars().insert(
                body = new_calendar
                ).execute()
            new_calendar_id = new_calendar['id']
            print('New calendar {} has been created!'.format(
                new_calendar['summary'])
                )
        else:
